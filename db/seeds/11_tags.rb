...
Tag.create(
  ...
  image_id: Image.where(url: "https://s3-us-west-1.amazonaws.com/jsg-flashcards/images/bfs.gif").pluck(:id).first
)

Tag.create(
  topic_id: Topic.where(title: "DFS").pluck(:id).first,
  image_id: Image.where(url: "https://s3-us-west-1.amazonaws.com/jsg-flashcards/images/dfs.gif").pluck(:id).first
)
